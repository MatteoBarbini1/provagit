      program deb_gis_bin5_mod_2_2_linux
* -------------------------- Linux version
...
c Point 1.3 Verifica calolo volumi in ingresso 
     ...
     1HYDROGRAPH NOT RESPECTED")')
        ...
        tt1_13 = omp_get_wtime()
!$OMP end single
!$OMP barrier  

!$OMP single private(i, j, icj,irj)
      tt1_14 = omp_get_wtime()
...
!$OMP atomic	
	dh_entrata_solido(ic1,ir1) = dh_entrata_solido(ic1,ir1) + 
     ...
     1then
      ...
      if(internalOutput(ic,ir).eq.1018)then
      write(1299,1929) ii,ic, ir,ic1,ir1, t, dh_cost(j),Q(j),DT,
     1 h(ic1,ir1),UU(j),coeff,peso(ic,ir,j),
     1h(ic,ir),senteta(ic,ir,j),Ch(ic,ir)
1929  format("grav",i8,4i6,11f14.4)
      endif
...
!$OMP atomic
     ...
     1then
       if (InternalOutputValle(ic1,ir1).eq.999.0) then
      if(internalOutput(ic,ir).eq.1018)then
      write(1299,1939) ii, ic, ir,ic1,ir1, t,ddh(j),peso_e(j),cq,DHH(j),
	1 DT,h(ic,ir),h(ic1,ir1),ele(ic,ir),ele(ic1,ir1)
1939  format("stra",i8,4i6,10f14.4)      
      endif
      dh_uscita_sez(ic,ir) = 
